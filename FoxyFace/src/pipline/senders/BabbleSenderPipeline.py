...
from blendshape_router.BabbleBuilder import BabbleBuilder
from blendshape_router.facades.babble.Babble import Babble
...
class BabbleSenderPipeline(SenderInterface):
    def __init__(self, config_manager: ConfigManager[Config], avatar_config_manager: ConfigManager[AvatarConfig]):
        ...
        self.__babble: Babble | None = None
    ...
    def close(self):
        ...
        with self.__create_lock:
            if self.__babble is not None:
                self.__babble.close()

    def __register_change_update(self) -> ConfigUpdateListener[Config]:
        watch_array: list[Callable[[Config], Any]] = [lambda config: config.sender.babble]

        return self.__config_manager.create_update_listener(lambda config: self.__babble_changed(),
                                                            watch_array, False)
    ...
    def __register_avatar_change_update(self) -> ConfigUpdateListener[AvatarConfig]:
        ...
        return self.__avatar_config_manager.create_update_listener(lambda config: self.__babble_changed(),
                                                                   watch_array, True)

    def __babble_changed(self):
        with self.__create_lock:
            if self.__babble is not None:
                self.__babble.close()
            ...
            if babble_config.solver_enabled:
                solver_model = ModelLoader(
                    PathUtil.to_path_or_default(babble_config.solver_model_path, SolverPath.get_default_asset_path()))

                clamped_percentage = max(0.0, min(1.0,
                                                  babble_config.solver_interleaved_vertices_percentage))
            ...
            self.__babble = (BabbleBuilder(HostAddress(ipaddress.ip_address(babble_config.ip),
                                                       babble_config.port))
                             .with_osc_cache_invalidate_timeout(babble_config.osc_cache_invalidate_timeout)
                             .with_osc_cache_full_sync_period(babble_config.osc_cache_full_sync_period)
                             .with_osc_cache_float_precision(babble_config.osc_cache_float_precision)
                             .with_osc_bundle_size(babble_config.osc_bundle_size)
                             .with_test_send_period(babble_config.test_send_period)
                             .with_test_animation_period(babble_config.test_animation_period)
                             .with_solver_model(solver_model)
                             .with_solver_threads(babble_config.solver_threads)
                             .with_solver_interleaved_vertices_count(vertices_count)
                             .with_solver_max_cps(babble_config.solver_max_cps)
            ...
            all_solver_inputs = frozenset(self.__babble.get_all_solver_input_functions())
            all_solver_outputs = frozenset(self.__babble.get_all_solver_output_functions())
