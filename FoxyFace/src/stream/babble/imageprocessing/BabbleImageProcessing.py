...
class BabbleImageProcessing(StreamReadOnly[ImageFrame]):
    ...
    def poll(self, timeout: float | None = None) -> ImageFrame:
        ...
        if self.__model_loader.model.is_input_rgb:
            img = mediapipe_frame.camera_frame.image.copy()
        else:
            img = cv2.cvtColor(mediapipe_frame.camera_frame.image, cv2.COLOR_RGB2GRAY)
        ...
        return ImageFrame(img, mediapipe_frame.camera_frame.timestamp_ns)
